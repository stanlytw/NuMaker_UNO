 ...
 ******************************************************************************/
//[2024-10-24]
//Note this driver does not involve RTOS. 
//The driver file is located in SampleCode/StdDriver/EMAC_xx/drv_emac
//The same name file can be found in SampleCode/Numaker_xxx/LwIP/drv_emac
//This might confuse user.
...
//[2024-10-15]Add extern variable. 
//Let m460_emac driver to access into LwIP netif->input. 
...
{        
    ...
    //[202-10-15]Copy from m460_emac LwIP-OS, for EMAC_ReceivePkt()
    /* Initial zero_copy rx pool */
    memp_init_pool(&memp_emac_rx);
    //[2024-10-16]comment to solve build error
...
uint32_t EMAC_ReceivePkt(void)
{
    ...
    {		
        ...
        //[2024-10-15]To fit STM32/LwIP, should call  
...
#endif
//[2024-10-15]Copy from m460_emac(lwip+OS)
uint32_t EMAC_ReceivePkt(void)
{
    uint32_t len = 0;
    PKT_FRAME_T* psPktFrame;
    struct pbuf *pbuf;
    //struct pbuf *my_pbuf;
	nu_emac_lwip_pbuf_t my_pbuf;
    if ( (len = synop_handle_received_data(&GMACdev, &psPktFrame)) > 0 )
    {		
        #if 1//[2024-10-18]comment to pass build error        
        //my_pbuf = (pbuf *)memp_malloc_pool(&memp_emac_rx);
        my_pbuf = memp_malloc_pool(&memp_emac_rx);
        if (my_pbuf != NULL)
        {
            my_pbuf->p.custom_free_function = NULL;//nu_emac_pbuf_free;//[2024-10-22]WC:must
            my_pbuf->psPktFrameDataBuf      = psPktFrame;
            //my_pbuf = psPktFrame;
            pbuf = pbuf_alloced_custom(PBUF_RAW,
                                       len,
                                       PBUF_REF,
                                       (struct pbuf_custom*)my_pbuf,
                                       psPktFrame,
                                       PKT_FRAME_BUF_SIZE);
            if (pbuf == NULL)
            {
